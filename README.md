...
Loon 无法完整表达 ABP 的请求类型、第一方/第三方和来源站点限制。为避免把局部规则扩大成整站拦截，带这些限定选项的网络规则会直接舍弃，不进行近似转换；`match-case` 会转换为对应的正则大小写行为，`important` 不改变匹配范围，可以安全忽略。详细数量见对应的统计文件。以下规则会舍弃：

- 元素隐藏等非网络规则；
- 带请求类型、第一方/第三方或来源站点等 Loon 无法准确表达的限定规则；
- 带上下文或资源类型选项的例外规则，避免将它们扩大成全局 `DIRECT`；
- `removeparam`、`redirect`、`csp`、`header` 等不是单纯拦截请求的修改器；
- 去掉 ABP 上下文选项后会退化成全局匹配的规则，例如 `*$ping,third-party`；
