#!/usr/bin/env python3
"""Convert EasyPrivacy's ABP network filters to Loon rule sets."""
...
DEFAULT_SOURCE_URL = "https://easylist-downloads.adblockplus.org/easyprivacy.txt"
...
def parse_metadata(lines: Iterable[str]) -> dict[str, str]:
    wanted = {"Title", "Version", "Last modified", "Checksum", "Homepage", "Licence"}
...
def header_lines(result: ConversionResult, source: str, kind: str) -> list[str]:
    ...
    lines.extend(
        [
            "# Generated by scripts/convert_easyprivacy.py; do not edit manually.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        "[Adblock Plus" not in text[:200]
        or "! Title: EasyPrivacy" not in text[:500]
        or len(text) < 10_000
    ):
        raise RuntimeError("input does not look like a complete EasyPrivacy filter list")
